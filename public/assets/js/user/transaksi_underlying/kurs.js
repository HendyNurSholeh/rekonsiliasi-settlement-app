/**
 * Currency (Kurs) functionality for Transaksi Underlying
 */

/**
 * Fetch all currencies
 * @param {string} tipeKurs 
 */
function fetchCurrencies(tipeKurs) {
    $.ajax({
        url: "/options/currencyConversionAPI",
        type: "POST",
        dataType: "json",
        data: {
            [csrfName]: csrfHash
...
/**
 * Filter currencies by transaction type
 * @param {string} tipeKurs 
 */
function filterCurrenciesByTipe(tipeKurs) {
    // Simpan value sebelumnya
    ...
    // Cek apakah prevValue masih ada di filtered
    if (filtered.some(c => c.id == prevValue)) {
        $('#kode_mata_uang').val(prevValue).trigger('change');
    } else {
        // Jika tidak ada, fallback ke USD
        $('#kode_mata_uang').val(getUsdCurrencyId()).trigger('change');
    }
}
...
function handleTransactionTypeChange() {
    $('#jenis_transaksi').on('change', function() {
        ...
        filterCurrenciesByTipe(tipeKurs);
        setTimeout(showKursInfo, 200); // update kurs info setelah filter
    });
}
...
function handleCurrencyChange() {
    $('#kode_mata_uang').on('change', function() {
        ...
        if (selectedCurrency && selectedCurrency != getUsdCurrencyId()) {
...
function convertToUsd() {
    ...
    if (!isNaN(nominalValue)) {
        $.ajax({
            url: "/getCurrencyRateToUsd/currencyConversionAPI",
            type: "GET",
            dataType: "json",
            data: ajaxData,
            success: function(response) {
                if (response.status === 200) {
                    ...
                    toastr["warning"]("Failed to fetch conversion rate");
                }
            },
            error: function(xhr, status, error) {
                toastr["error"]("Error fetching conversion rate");
            }
        });
    }
}
