...
function renderStatusEscrow(status) {
	...
	if (statusLower === "sukses") {
		...
		statusLower === "belum proses" ||
		statusLower === "sukses sebagian"
	) {
...
function initializeDataTable() {
	jurnalEscrowBillerPlTable = $("#jurnalEscrowBillerPlTable").DataTable({
		...
		ajax: {
			url:
				...
				"settlement/jurnal-escrow-biller-pl/datatable",
			type: "GET",
			data: function (d) {
	...
	$("#jurnalEscrowBillerPlTable tbody").on(
		"click",
		"tr.parent-row",
		function (e) {
	...
	$("#jurnalEscrowBillerPlTable tbody")
...
function searchInDetails(searchTerm) {
	...
	jurnalEscrowBillerPlTable.rows().every(function () {
		...
		const hasMatch = childData.some(
			(child) =>
				(child.d_STATUS_KR_ESCROW &&
					child.d_STATUS_KR_ESCROW.toLowerCase().includes(searchLower)) ||
				(child.d_NO_REF &&
