// ==========================================
// JURNAL ESCROW BILLER PL - BATCH PROCESS MODULE
// ==========================================
...
function reloadDatatableForError() {
    if (typeof jurnalEscrowBillerPlTable !== 'undefined') {
        setTimeout(function() {
...
function processBatchJurnal(kdSettle) {
    ...
    $.ajax({
        url: window.appConfig.baseUrl + "settlement/jurnal-escrow-biller-pl/proses",
        type: 'POST',
        ...
        error: function(xhr, status, error) {
            ...
            let errorMessage = 'âŒ Terjadi kesalahan saat batch process!';
            
            if (xhr.status === 403 || xhr.status === 419) {
                errorMessage = 'âŒ Session expired. Silakan refresh halaman dan coba lagi.';
                ...
                errorMessage = 'âŒ Koneksi terputus. Periksa koneksi internet Anda.';
            } else if (xhr.status >= 500) {
                errorMessage = 'âŒ Server error. Silakan coba lagi atau hubungi administrator.';
            } else if (xhr.responseText) {
                try {
                    const errorResponse = JSON.parse(xhr.responseText);
                    errorMessage = `âŒ Error: ${errorResponse.message || error}`;
                } catch (e) {
                    errorMessage = `âŒ Error: ${xhr.responseText}`;
                }
            }
