...
function initializeDataTable() {
		...
		columns: [
			{
				...
				render: function (data, type, row, meta) {
					if (row.has_children) {
						return (
							'<i class="fal fa-plus-square expand-btn text-purple" ' +
			...
			},
			{
				...
				render: function (data, type, row) {
					return (
						'<strong class="text-purple">' + formatCurrency(data) + "</strong>"
					);
				},
			},
			{
				...
				render: function (data, type, row) {
					return (
						'<span class="badge" style="background-color: #6c5190; color: white;">' +
						(data || "0") +
						"</span>"
					);
				},
			...
			},
			{
				...
				render: function (data, type, row) {
					if (row.child_count > 0) {
						return (
							'<small class="text-muted">' + row.child_count + " detail</small>"
						);
					}
	...
	$("#jurnalCaEscrowTable tbody").on("click", "tr.parent-row", function (e) {
	...
	$("#jurnalCaEscrowTable tbody")
		.on("mouseenter", "tr.parent-row", function () {
	...
	$(document).on("click", ".child-details-container button", function (e) {
		...
		if ($(".btn-processing").length > 0 && !$btn.hasClass("btn-processing")) {
			...
			showAlert(
				"warning",
				"Ada transaksi lain yang sedang diproses. Silakan tunggu hingga selesai."
			);
			return false;
		}
	});
	...
	$(document).on("click", "#modalCallbackLog .btn-detail", function() {
		const logId = $(this).data("id");
		showCallbackDetailModal(logId);
	});
}
...
function initializeTooltips() {
	if (typeof $().tooltip === "function") {
		$("[title]").tooltip({
			placement: "right", // Paksa tooltip di kanan
			container: "body", // Render di body agar tidak terpotong
...
function formatChildRows(childData, kdSettle) {
	...
	const errorMessage =
		childData.find((c) => c.d_ERROR_MESSAGE?.trim())?.d_ERROR_MESSAGE || "";
	...
	// Build table rows
	const tableRows = childData
		.map((child) => {
			const noRef = child.d_NO_REF || "-";

			const rowData = [
				noRef !== "-" 
					? `<code class="ref-link text-primary" style="cursor: pointer;" onclick="showCallbackLogModalByRef('${noRef}')">${noRef}</code>`
					: `<code>${noRef}</code>`,
				...
				`<strong class="text-dark">${formatCurrency(
					child.d_AMOUNT || 0
				)}</strong>`,
				renderCoreResCode(child.d_CODE_RES),
				child.d_CORE_REF
					? `<code>${child.d_CORE_REF}</code>`
					: '<span class="text-muted">NULL</span>',
				child.d_CORE_DATETIME
					? `<small>${child.d_CORE_DATETIME}</small>`
					: '<span class="text-muted">NULL</span>',
	...
	const headerCells = tableHeaders
		.map((header, idx) => `<th style="width: ${colWidths[idx]}">${header}</th>`)
		.join("");
...
function searchInDetails(searchTerm) {
	...
	jurnalCaEscrowTable.rows().every(function () {
		...
		const hasMatch = childData.some(
			(child) =>
				(child.d_NO_REF &&
					child.d_NO_REF.toLowerCase().includes(searchLower)) ||
				(child.d_DEBIT_ACCOUNT &&
					child.d_DEBIT_ACCOUNT.toLowerCase().includes(searchLower)) ||
				(child.d_DEBIT_NAME &&
					child.d_DEBIT_NAME.toLowerCase().includes(searchLower)) ||
				(child.d_CREDIT_ACCOUNT &&
					child.d_CREDIT_ACCOUNT.toLowerCase().includes(searchLower)) ||
				(child.d_CREDIT_NAME &&
					child.d_CREDIT_NAME.toLowerCase().includes(searchLower)) ||
				(child.d_CODE_RES &&
					child.d_CODE_RES.toLowerCase().includes(searchLower)) ||
				(child.d_CORE_REF &&
					child.d_CORE_REF.toLowerCase().includes(searchLower)) ||
				(child.d_CORE_DATETIME &&
					child.d_CORE_DATETIME.toLowerCase().includes(searchLower))
		);
	...
	if (found) {
		showAlert(
			"info",
			`Ditemukan hasil pencarian untuk "${searchTerm}". Baris yang relevan telah diperluas.`
		);
		setTimeout(() => initializeTooltips(), 300);
	} else {
		showAlert(
			"warning",
			`Tidak ditemukan hasil untuk "${searchTerm}" dalam detail transaksi.`
		);
	}
}
...
function refreshTableData() {
	if (jurnalCaEscrowTable) {
		console.log(
			"Manual refresh with preserved expand state for:",
