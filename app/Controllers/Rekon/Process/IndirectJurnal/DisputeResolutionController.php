...
class DisputeResolutionController extends BaseController
{
    ...
    public function datatable()
    {
        ...
        // Debug log
        log_message('info', 'Indirect Dispute DataTable parameters - Tanggal: ' . $tanggalRekon . ', Status Biller: ' . $statusBiller . ', Status Core: ' . $statusCore);
    ...
    /**
     * Get indirect dispute detail for modal
     */
    ...
    public function update()
    {
        ...
        // Debug log
        log_message('info', 'UpdateIndirectDispute - v_ID: ' . $vId . ', Status Biller: ' . $statusBiller . ', Status Core: ' . $statusCore . ', Status Settlement: ' . $statusSettlement . ', IDPartner: ' . $idpartner);

        if (!$vId || $statusBiller === null || $statusCore === null || $statusSettlement === null || !$idpartner) {
            return $this->response->setJSON([
                'success' => false,
        ...
        try {
            ...
            $query = $db->query("CALL p_update_dispute_tx(?, ?, ?, ?, ?)", [
                $vId, $statusBiller, $statusCore, $statusSettlement, $idpartner
            ]);
            
            // Log the update activity
            $this->logActivity([
                'log_name' => LogEnum::DATA,
                'description' => 'Update Data Button Proses Rekonsiliasi/Indirect Jurnal/Penyelesaian Dispute',
                'event' => EventLogEnum::UPDATED,
                'subject_id' => $vId,
                'subject' => 'Menu: Proses Rekonsiliasi/Indirect Jurnal/Penyelesaian Dispute',
            ...
            log_message('info', 'p_update_dispute_tx called successfully for v_ID: ' . $vId);
            ...
            return $this->response->setJSON([
                'success' => false,
                'message' => 'Terjadi kesalahan saat memproses data dispute: ' . $e->getMessage(),
                'csrf_token' => csrf_hash()
            ]);
        }
    }
